  ...
  group('PhoneUtils Tests', () {
    group('normalizePhone', () {
      test('normalizePhone removes spaces', () {
        final result = PhoneUtils.normalizePhone('050 123 4567');
        expect(result, '0501234567');
      });

      test('normalizePhone removes dashes and plus', () {
        final result = PhoneUtils.normalizePhone('+966-50-123-4567');
      ...
      test('normalizePhone handles empty string', () {
        final result = PhoneUtils.normalizePhone('');
        expect(result, '');
      });

      test('normalizePhone removes all non-digit characters', () {
        final result = PhoneUtils.normalizePhone('050-123-4567 ext. 123');
      ...
      test('normalizePhone handles mixed formatting', () {
        final result = PhoneUtils.normalizePhone('+1 (800) 555-1234');
        expect(result, '18005551234');
      });

      test('normalizePhone handles letters', () {
        final result = PhoneUtils.normalizePhone('abc123def');
        expect(result, '123');
      });

      test('normalizePhone handles only non-digits', () {
        final result = PhoneUtils.normalizePhone('abc-def-ghi');
        expect(result, '');
      });

      test('normalizePhone preserves digits only', () {
        final result = PhoneUtils.normalizePhone('0501234567');
        expect(result, '0501234567');
      });
    });
    ...
    group('Integration Tests', () {
      test('normalizePhone and phoneToFakeEmail work together', () {
        ...
        final normalized = PhoneUtils.normalizePhone(formattedPhone);
        final email = PhoneUtils.phoneToFakeEmail(normalized);
